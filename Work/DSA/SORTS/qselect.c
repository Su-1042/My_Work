...
int FindIndex(int *arr, int to_find, int lower, int upper)
{
...
void QuickPartition(int *arr, int pivot, int lower, int upper)
{
    int i = lower;
    int j = upper - 1;
    swap(&arr[upper], &arr[pivot]);
...
void QSDriver(int *arr, int lower, int upper)
{
    ...
    {
        int pivot_value = median_of_3(arr, lower, upper - lower + 1);
        int pivot_index = FindIndex(arr, pivot_value, lower, upper);
        QSDriver(arr, lower, pivot_index - 1);
        QSDriver(arr, pivot_index + 1, upper);
        QuickPartition(arr, pivot_index, lower, upper);
    }
}
